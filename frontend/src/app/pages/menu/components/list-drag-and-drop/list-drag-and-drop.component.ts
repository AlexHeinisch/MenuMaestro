...
export class ListDragAndDropComponent {
  ...
  lists = [
    {
      id: 1,
      name: "Planned",
      items: [{ id: 9, name: "Lunch: Caesar's Salad" }],
      collapsed: false,
    }, // New list added first
    {
      id: 2,
    ...
    {
      id: 3,
      name: "Day 2 Meal Plan",
      items: [{ id: 3, name: "Day 2 Dinner: Spaghetti" }],
      collapsed: false,
    },
    {
      id: 4,
    ...
    },
    {
      ...
      items: [{ id: 6, name: "Breakfast: Cereal" }],
      collapsed: false,
    },
    {
      id: 6,
      name: "Ready To Cook",
      items: [{ id: 7, name: "Lunch: Meatballs" }],
      collapsed: false,
    },
    {
      id: 7,
      name: "Cooked",
      items: [{ id: 8, name: "Dinner: Pizza" }],
      collapsed: false,
    },
  ];
  ...
  drop(event: CdkDragDrop<{ id: number; name: string }[]>, listIndex: number) {
    ...
    if (targetList.name.includes("Day")) {
      ...
      const targetDayPrefix =
        targetList.name.split(" ")[0] + " " + targetList.name.split(" ")[1]; // 'Day 2'

      // If the item's name already contains 'Day X', replace it with the new prefix
      if (currentName.match(/Day \d+/)) {
        event.container.data[event.currentIndex].name = currentName.replace(
          /Day \d+/,
          targetDayPrefix,
        );
      } else {
