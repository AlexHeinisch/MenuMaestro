...
export class ShoppingListAddItemComponent {
  ...
  onIngredientSelected(selected: string) {
    ...
    if (selectedIngredient) {
      this.shoppingListIngredientAddDto.existingIngredientId =
        selectedIngredient.id;
      this.shoppingListIngredientAddDto.unit =
        selectedIngredient.defaultUnit as IngredientUnitDto;
    }
      ...
      this.shoppingListApiService
        ...
        .subscribe({
          next: (response) => {
            ...
            this.toastr.success(
              "Successfully added item to your shopping list!",
            );
          },
          error: (error) => {
            this.errorService.printErrorResponse(error);
