...
export class ShoppingListsOverviewComponent {
  ...
  fetchShoppingListForSearchSuggestion(): void {
    this.shoppingListApiService
      ...
      .subscribe({
        next: (data) => {
          this.shoppingListSearchSuggestions = data.content.map(
            (item) => item.name,
          );
        },
        error: (err) => {
  ...
  fetchMenusForSearchSuggestion() {
    this.menusApiService
      .getMenus(0, 5, undefined, this.menuSearchTerm)
      .subscribe({
        next: (response: MenuSummaryListPaginatedDto) => {
          if (response.content) {
            this.menuSearchResults = response.content;
            this.menuSearchSuggestions = this.menuSearchResults.map(
              (menu) => menu.name,
            );
          } else {
  ...
  onMenuSelected(selected: string) {
    ...
    this.menuIdOfSearchTerm = this.menuSearchResults.find(
      (menu) => menu.name === this.menuSearchTerm,
    )?.id;
    this.fetchShoppingLists();
  }
